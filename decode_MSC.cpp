...
double MinSumCAlgorithm(double* LLR_Pi1, double** qij1, int m, int** R, int** C, double** rji0, int n, int maxdegree, int maxcoldegree, int* c_, int* r_column, int* q_column, double* LQ, double* temp_row)
{
	int i, j, l, q;
	double Qi = 0;
	double sigma;//sigma = 連加的符號Σ
	//double pai;//pai = 連乘符號Π
	//double phai;//phai = Φ
	//double fabs;
	//double min;
	double LLR_rji;
	...
	for (j = 0; j < m; j++)
	{
		for (l = 0; l < maxdegree; l++)
		{
			...
			for (q = 0; q < maxdegree; q++)
				if (avoid_two_cycle_usage == 0)//還原2-cycle訊息 by willy
				{
	...
	for (j = 0; j < n; j++)
	{
		for (l = 0; l < maxcoldegree; l++)
		{
			if (C[j][l] != 0)
			{
				...
				{
					sigma += rji0[j][i];//rji0存的是c-v messages						
				}
				Qi = sigma;
				if (avoid_two_cycle_usage == 1) //還原2-cycle訊息
				{
					sigma -= rji0[j][l];
				}
				sigma += LLR_Pi1[j];
	...
	for (Qnumber = 0; Qnumber < n; Qnumber++)
	{
		...
		for (j = 0; j < n; j++)
		{
			for (l = 0; l < maxcoldegree; l++)
			{
				sigma = 0;
				for (i = 0; i < maxcoldegree; i++)
					sigma += temp_rji0[j][i];//rji0存的是c-v messages						
				sigma -= temp_rji0[j][l]; //2-cycle扣除
				sigma += LLR_Pi1[j]; //加入初始機率
				//store

...
void sumproduct3(double* row, double* rji, int maxdegree, int Qnumber, int j, int l, int** R, int** C, int n, int m, double* Pi, int maxcoldegree) {
    ...
    // 舊版策略：檢查此 check node 是否連到根節點 Qnumber。
    for (i = 0; i < maxdegree; i++) {
        if (R[j][i] == 0) continue;		// 略過不規則矩陣中的補零空位

        // 排除本次輸出邊 l，只檢查其他相鄰 variable node。
    ...
    // 舊版策略：若命中根節點，直接中性化此 check-to-variable 訊息。
    if (has_root_source) {
        // 此區塊被 #if 0 包住，不會參與目前編譯。
        *rji = 0.0;  
        return; 
    }

    // 未命中時，計算標準 SPA check-node product。
    for (i = 0; i < maxdegree; i++) {
        if (i != l) {
            if (R[j][i] == 0) continue;		// 略過補零空位

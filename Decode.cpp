		...
		for (j = 0; j < m; j++)
		{
			for (l = 0; l < maxdegree; l++)
			{
				...
				//}//for (k=0;k<maxdegree;k++)	
					...
					//store
				//printf("rji:%f \n", product_rji);
				temp_rji0[R[j][l] - 1][r_column[R[j][l] - 1]] = 0.5 + 0.5 * product_rji;	   //?´Â??¥Êñ∞ÂæåÁ? r Ë®äÊÅØ
...
void sumproduct3(double* row, double* rji, int maxdegree, int Qnumber, int j, int l, int** R, int** C, int n, int m, double* Pi, int maxcoldegree) {
    ...
    for (i = 0; i < maxdegree; i++) {
        if (i != l) {
            ...
            // 3. Hybrid SPA ?∏Â??èËºØÔºöÁ≤æÊ∫ñÊ???
            if (is_4cycle) {
                // Â¶ÇÊ??ôÂÄãÁî¢??4-cycle ?ÑÁ?ÈªûÊòØ?±È?ÊØíÁò§ÔºåÂ??¥Êé•?®Ê??¥ÂÄãÊ™¢?•ÊñπÁ®ãÂ? (?êÊó©ÁµêÊ?‰∏¶Áµ¶‰∫?0.0)
                if (is_hotspot(v_neighbor)) {
                    *rji = 0.0;
                    return; // ?¥Êé•‰∏≠Êñ∑?ôÂÄ?functionÔºåÊç®Ê£ÑÈÄôÂÄãÊ™¢?•Á?ÈªûÂÇ≥?ûÂá∫?ªÁ?Ë®äÊÅØ
                } else {
                    // Â¶ÇÊ??™ÊòØ?∂ÁÑ∂?¢Á? 1 Ê¨?4-cycle ?ÑÊôÆ?öÁ?ÈªûÔ??æÈ?ÂÆÉÔ?Á∂≠Ê?Ê®ôÊ? SPA Â§ñÂú®Ë®äÊÅØË®àÁ?
                    product *= (1.0 - 2.0 * row[i]);
                }
            } else {
                // Ê≠?∏∏?ÑÈ?Ëø¥Â?Ë∑ØÂ?ÔºåÁ∂≠?ÅÊ?Ê∫?SPA Â§ñÂú®Ë®äÊÅØË®àÁ?
                product *= (1.0 - 2.0 * row[i]);
            }
        }
    }
